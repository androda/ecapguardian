...
void read_config(const char *configfile, int type)
{
    ...
    if (!o.read(configfile, type))    {
        syslog(LOG_ERR, "%s", "Error parsing the ecapguardian.conf file or other ecapguardian configuration files");
        std::cerr << "Error parsing the ecapguardian.conf file or other ecapguardian configuration files" << std::endl;
	std::cerr << "Config file specified: " << configfile << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    openlog("ecapguardian", LOG_PID | LOG_CONS, LOG_USER);
    ...
    for (int i = 1; i < argc; i++)    {
        if (argv[i][0] == '-')        {
            for (unsigned int j = 1; j < strlen(argv[i]); j++)            {
                ...
                case 'v':
                    std::cout << "ecapguardian " << PACKAGE_VERSION << std::endl << std::endl
                ...
                case 'h':
                    ...
                    std::cout << "  -q causes ecapguardian to kill any running copy." << std::endl;
...
//		return 1;
    ...
    if (DANS_MAXFD > FD_SETSIZE)    {
        ...
        std::cerr << "ecapguardian compiled with with-filedescriptors=" << DANS_MAXFD << std::endl;
    ...
    if ((o.max_subprocs + o.prefork_subprocs) > max_maxsubprocs)    {
        syslog(LOG_ERR, "%s", "maxsubprocs option in ecapguardian.conf has a value too high.");
        std::cerr << " maxsubprocs option in ecapguardian.conf has a value too high for current file id limit (" << rlim.rlim_cur << ")"<< std::endl;
    ...
    if ((sg = getgrnam(o.daemon_group_name.c_str())) != 0)    {
        ...
        syslog(LOG_ERR, "Check the group that ecapguardian runs as (%s)", o.daemon_group_name.c_str());
        std::cerr << "Unable to getgrnam(): " << strerror(errno) << std::endl;
        std::cerr << "Check the group that ecapguardian runs as (" << o.daemon_group_name.c_str() << ")" << std::endl;
        return 1;
    }
    ...
    while (true)    {
        ...
        if (rc == 2)        {
            ...
            if (!o.read(configfile.c_str(), 2))            {
                syslog(LOG_ERR, "%s", "Error re-parsing the ecapguardian.conf file or other ecapguardian configuration files");
#ifdef DGDEBUG
                std::cerr << "Error re-parsing the ecapguardian.conf file or other ecapguardian configuration files" << std::endl;
#endif
                return 1;
