...
public:
	...
	int in(DataBuffer * d, BaseSocket * sock, BaseSocket * peersock, HTTPHeader * requestheader,
...
int dminstance::in(DataBuffer * d, BaseSocket * sock, BaseSocket * peersock, class HTTPHeader * requestheader,
...
//      std::cout << "cvtest:" << cv["dummy"] << std::endl;
...
#ifdef DGDEBUG
	std::cout << getpid() << "blocksize: " << blocksize << std::endl;
#endif
	...
	while ((bytesremaining > 0) || geteverything) {
...
#ifdef DGDEBUG
			std::cout << getpid() << "o.trickle_delay > 0" << std::endl;
#endif
...
#ifdef DGDEBUG
				std::cout << getpid() << "doneinitialdelay ? nowadays.tv_sec - ..." << std::endl;
#endif
...
#ifdef DGDEBUG
				std::cout << getpid() << "trickle delay - sending X-DGKeepAlive: on" << std::endl;
#endif
		...
		if (wantall) {
#ifdef DGDEBUG
			std::cout << getpid() << "wants ALL" << std::endl;
#endif
			if (!swappedtodisk) {
#ifdef DGDEBUG
			std::cout << getpid() << "!swappedtodisk" << std::endl;
#endif
		...
		if (!swappedtodisk) {
#ifdef DGDEBUG
			std::cout << getpid() << "!swappedtodisk 2" << std::endl;
#endif
