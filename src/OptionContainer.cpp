...
bool OptionContainer::read(const char *filename, int type)
{
    ...
    try    {
        ...
        min_subprocs = findoptionI("minsubprocs");
        if (!realitycheck(min_subprocs, 1, max_subprocs-1, "minsubprocs"))        {
            return false;
        }		// check its a reasonable value
        maxspare_subprocs = findoptionI("maxsparesubprocs");
        if (!realitycheck(maxspare_subprocs, min_subprocs, max_subprocs, "maxsparesubprocs"))        {
            return false;
        }		// check its a reasonable value
        prefork_subprocs = findoptionI("preforksubprocs");
        if (!realitycheck(prefork_subprocs, 1, max_subprocs, "preforksubprocs"))        {
            return false;
        }		// check its a reasonable value
        minspare_subprocs = findoptionI("minsparesubprocs");
        if (!realitycheck(minspare_subprocs, 0, maxspare_subprocs-1, "minsparesubprocs"))        {
            return false;
        }		// check its a reasonable value
        maxage_subprocs = findoptionI("maxagesubprocs");
        if (!realitycheck(maxage_subprocs, 1, 0, "maxagesubprocs"))        {
            return false;
        }		// check its a reasonable value

        gentle_chunk = findoptionI("gentlechunk");
        if (gentle_chunk > 0)        {
            if (!realitycheck(gentle_chunk, 1, min_subprocs, "gentlechunk"))            {
                return false;
            }		// check its a reasonable value
        }        else        {
            gentle_chunk = prefork_subprocs;
        }
        monitor_start = 0;
        ...
        if (monitor_helper == "")        {
            ...
            if (!realitycheck(monitor_start, 0, min_subprocs, "monitorstart"))            {
                return false;
            }		// check its a reasonable value
            if (monitor_start == 0) monitor_start = min_subprocs;
        }
        ...
        */
        ...
        if (findoptionS("logsubprocesshandling") == "on")        {
            logsubprocs = true;
        }        else        {
            logsubprocs = false;
        }
